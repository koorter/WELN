...
def evaluate(qf, ql, gf, gl):
    ...
    # score 是否可理解为当前余弦距离的排序？     余弦距离越大，相似度越大
...
def extract_feature(model, dataloaders, view_index=1):  # view_index
...
# def eval_and_test():              # 仿照美玉的写的，已弃用
#     data_transforms = transforms.Compose([
#         transforms.Resize((get_yaml_value("image_size"), get_yaml_value("image_size")), interpolation=InterpolationMode.BICUBIC),
#         transforms.ToTensor(),
#         transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
#     ])
#
#     image_datasets = {x: datasets.ImageFolder(os.path.join("/media/sues/daa8aa38-6c2b-4fb6-a66f-327e4fc2f6a6/dataset/University-Release/test", x), data_transforms) for x in
#                       ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
#     dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=64,
#                                                   shuffle=False, num_workers=16) for x in    # num_workers=16是
#                    ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
#     print("Testing Start >>>>>>>>")
...
#                                    get_yaml_value('name'))) and len(save_model_list) >= 3:
...
def eval_and_test():            # LPN
    ...
    image_datasets = {x: datasets.ImageFolder(os.path.join("/media/sues/daa8aa38-6c2b-4fb6-a66f-327e4fc2f6a6/dataset/University-Release/test", x), data_transforms) for x in
                      ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=64,
                                                  shuffle=False, num_workers=32) for x in    # num_workers=16是
                   ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
    print("Testing Start >>>>>>>>")
    table_path = os.path.join(get_yaml_value("weight_save_path"),
                              get_yaml_value("name") + ".csv")
    ...
    if os.path.exists(os.path.join(get_yaml_value("weight_save_path"),
        ...
        for query in ['drone', 'satellite']:
            for seq in range(-3, 0):
                model, net_name, num_directions = load_network(seq=seq)
                print(net_name, block, num_directions)

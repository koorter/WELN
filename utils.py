...
def load_network(seq):
    ...
    num_directions = get_yaml_value("num_directions")
    # model = model_.model_dict[model_name](class_num=classes, drop_rate=drop_rate)
    model = model_.model_dict[model_name](class_num=classes, drop_rate=drop_rate, block=block)
    # model = model_.ResNet(classes, drop_rate)
    model.load_state_dict(torch.load(os.path.join(dirname, last_model_name)))
    return model, last_model_name, num_directions
...
def summary_csv_extract_pic(csv_path):
    ...
    for pic in query_pic:
        ...
        for gl_path in gallery_list:
            ...
            dest_name = os.path.dirname(src_name) + os.sep + str(count) + "_" + gl_path.split("/")[-2] + "." + \
                        gl_path.split(".")[-1]
            print(src_name)
            print(dest_name)
